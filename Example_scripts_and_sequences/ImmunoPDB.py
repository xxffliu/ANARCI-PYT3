...
def pairwise_muscle(seq1, seq2,exact=True):
    ...
    if len(result) == 3:
        ...
        sys.stderr.write("Problem parsing output from muscle: %s"%output[0])
...
if __name__ == '__main__':
    ...
    if args.receptor == 'ig':
        ...
        if args.scheme not in ['imgt','i','aho','a']: 
            sys.stderr,write('Only imgt or aho schemes can be applied to tcrs')
            sys.exit(1)
    ...
    try:        
        ...
        sys.stderr.write('File %s could not be opened'%args.inputstructure)
        sys.exit(1)
    except Exception as e:
        sys.stderr.write('%s could not be parsed:'%args.inputstructure, e)
        sys.exit(1)
    ...
    try:
        if args.outfile:
            ...
            of = sys.stdout
        of.write('REMARK   6 SCHEME %s'%args.scheme.upper())
        of.write('REMARK   6 ANARCI TYPE, PAIRING AND ASSIGNED GERMLINE DETAILS')
        ...
        sys.stderr.write('Could not write to file %s'%(args.outfile))
        sys.exit(1)
    except Exception as e:
        sys.stderr.write(e)
        sys.exit(1)
