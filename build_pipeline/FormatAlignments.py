    ...
    with open( filename, "w" ) as outfile:
        for al in sequences:
            for s in sequences[al]:
                outfile.write(">%s|%s|%s|%s\n"%tuple( list(al)+list(s) ))
                outfile.write(sequences[al][s] + '\n')
    return filename
...
def output_python_lookup(all_gene_alignments, path=None):
    ...
    with open(filename,'w') as outfile:
        outfile.write("all_germlines = "+repr(all_gene_alignments) + '\n')
...
def write_stockholm( sequences, ID, outfile):
        ...
        for s in sequences:
            outfile.write(s.replace(" ", "_").ljust(pad_length) + sequences[s].replace(".","-") + '\n')
        outfile.write("#=GC RF".ljust(pad_length) + "x"*len(sequences[s]) + '\n')
        outfile.write("//\n")
